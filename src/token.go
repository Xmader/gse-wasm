...
func (token *Token) Text() string {
	return textSliceToString(token.Texts)
}

// GetSegments 该分词文本的进一步分词划分，比如 "山达尔星联邦共和国联邦政府" 这个分词
...
func (token *Token) Equals(str string) bool {
	...
	for i := 0; i < len(token.Texts); i++ {
		textArray := []byte(token.Texts[i])
